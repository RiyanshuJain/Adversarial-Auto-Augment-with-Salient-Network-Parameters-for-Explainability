...
parser.add_argument('--model', default='resnet18', type=str, help='name of architecture')
...
parser.add_argument('--image_target_label', default=None, type=int, help='image label (number from 0 to 100 according to CIFAR100 labels)')
...
def save_gradients(grads_to_save, args, experiment, reference_image, inv_transform_test):
    ...
    grads_to_save = grads_to_save.detach().cpu().numpy().reshape((32, 32))
    ...
    # save_path = os.path.join('figures_pes', args.figure_folder_name)
    save_path = os.path.join('figures_lpa3', args.figure_folder_name)
...
if __name__ == '__main__':
    ...
    if images_path != '<insert-ImageNet-val-path-here>':
        ...
        print("""
               Image validation set path is not specified.
    ...
    net = torch.load("./model_25_pes.pth")
    # net.load_state_dict(torch.load("./model_lpa3_pes.pth"))
    net = net.to(device)
    net.eval()
    ...
    # inference_file = "./val_inf/pes/val_inf_rn18_{}.pth".format(args.image_path.split('/')[-1].split('.')[0])
    inference_file = "./val_inf/lpa3/val_inf_rn18_{}.pth".format(args.image_path.split('/')[-1].split('.')[0])
    ...
    # filter_stats_file = "./val_saliency/pes/val_saliency_rn18_{}.pth".format(args.image_path.split('/')[-1].split('.')[0])
    filter_stats_file = "./val_saliency/lpa3/val_saliency_rn18_{}.pth".format(args.image_path.split('/')[-1].split('.')[0])
    ...
    save_name = str(args.reference_id) if args.reference_id is not None else args.image_path.split('/')[-1].split('.')[0]
    save_name += '_' + args.model
    # fig.savefig('figures_pes/filter_saliency_{}.png'.format(save_name))
    fig.savefig('figures_lpa3/filter_saliency_{}.png'.format(save_name))
    # print('Filter saliency saved to figures_pes/filter_saliency_{}.png'.format(save_name))
    print('Filter saliency saved to figures_lpa3/filter_saliency_{}.png'.format(save_name))
